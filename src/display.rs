...
pub fn print_help() {
    ...
    println!("    ports free <port(s)>       Kill port(s); accepts list + ranges (3000-3010)");
    println!("    ports kill-port <port(s)>  Alias for `free` (same as npx kill-port)");
    println!("    ports kill <pid(s)>        Kill process(es) by PID");
    println!("    ports killall <name>       Like `killall node` — kill all procs by name (confirms)");
    println!("    ports kill-project <name>  Kill every port belonging to a project");
    println!("    ports nuke                 Kill every dev server (confirms first)");
    println!("    ports run <cmd...>         Kill listening ports in $PWD's project, then exec <cmd>");
    ...
    println!();
    println!("  {}", "FLAGS".bold());
    println!("    -f, --force       Skip SIGTERM, go straight to SIGKILL");
    println!("    -F, --full        (killall only) match against the full command line, not just the name");
    println!();
    println!(
        "  {} {}",
        "repo:".dimmed(),
